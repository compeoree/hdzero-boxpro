...
static void mplayer_create_slider(lv_obj_t *parent, int16_t x, int16_t y) {
    ...
    lv_obj_set_pos(controller._slider, x, y + 16);
    ...
    lv_obj_set_style_text_font(controller._label, &lv_font_montserrat_14, 0);
    lv_obj_set_style_text_align(controller._label, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_set_style_pad_top(controller._label, 8, 0);
    ...
    lv_obj_set_size(controller._label, 100, MPLAYER_BTN_HEIGHT);
}
...
static void init_mplayer() {
    ...
    lv_obj_set_pos(controller.bar, (MPLAYER_SCR_WIDTH - MPLAYER_CB_WIDTH) >> 1, MPLAYER_SCR_HEIGHT - 100);
