...
static void main_menu_create_entry(lv_obj_t *menu, lv_obj_t *section, page_pack_t *pp) {
    ...
    lv_obj_set_style_text_font(cont, &lv_font_montserrat_18, 0);
...
void main_menu_init(void) {
    ...
    page_packs[i++] = &pp_osd;
    page_packs[i++] = &pp_power;
    page_packs[i++] = &pp_fans;
    page_packs[i++] = &pp_record;
    page_packs[i++] = &pp_autoscan;
    if (g_setting.is_pro) {
        page_packs[i++] = &pp_elrs;
        page_packs[i++] = &pp_wifi;
    }
    page_packs[i++] = &pp_headtracker;
    page_packs[i++] = &pp_playback;
    page_packs[i++] = &pp_storage;
    pp_version.p_arr.max = ROW_PAGE_VERSION_COUNT;
    page_packs[i++] = &pp_version;
    page_packs[i++] = &pp_focus_chart;
    page_packs[i++] = &pp_clock;
    page_packs[i++] = &pp_input;
    page_packs[i++] = &pp_sleep;
    ...
    lv_obj_set_size(menu, lv_disp_get_hor_res(NULL) - 300, lv_disp_get_ver_res(NULL) - 48);
    lv_obj_set_pos(menu, 150, 48);
    ...
    lv_obj_set_size(root_page, 240, 672);
    ...
    lv_obj_set_style_bg_color(progress_bar.bar, lv_color_hex(TEXT_COLOR_DEFAULT), LV_PART_INDICATOR);
    lv_obj_set_size(progress_bar.bar, 210, 12);
