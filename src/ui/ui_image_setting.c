...
static void ims_page_init(uint8_t *val) {
    ...
    snprintf(buf, sizeof(buf), "%s:", _lang("LCD"));
    ...
    snprintf(buf, sizeof(buf), "LCD %s:", _lang("Auto Off"));
...
void ims_init(void) {
    ...
    lv_obj_align(canvas_ims, LV_ALIGN_BOTTOM_MID, 0, -IMS_CANVAS_HEIGHT / 2);
    lv_obj_add_flag(canvas_ims, LV_OBJ_FLAG_HIDDEN);
    lv_draw_line_dsc_init(&line_dsc);
    lv_draw_label_dsc_init(&label_dsc);
    label_dsc.font = &lv_font_montserrat_14; // LV_FONT_DEFAULT;
    ims_page_init(defs);
    g_bShowIMS = false;

    LCD_Brightness(defs[0]);
...
void change_oled_brightness(uint8_t key) {
    if (key == DIAL_KEY_UP) {
        if (g_setting.image.oled != MAX_LCD_BRIGHTNESS) {
            g_setting.image.oled += 1;
        } else {
            return;
        }
    } else if (key == DIAL_KEY_DOWN) {
        if (g_setting.image.oled != MIN_LCD_BRIGHTNESS) {
            g_setting.image.oled -= 1;
        } else {
            return;
        }
