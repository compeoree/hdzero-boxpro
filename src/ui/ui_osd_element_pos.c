...
static lv_coord_t col_dsc[] = {6, 63, 80, 80, 40, 20, LV_GRID_TEMPLATE_LAST};
static lv_coord_t row_dsc[] = {26, 26, 26, 26, 26, 26, 26, 26, 26, LV_GRID_TEMPLATE_LAST};
...
static void fill_osd_elements_str() {
    ...
    if (g_setting.storage.selftest) {
        max_element = OSD_GOGGLE_TEMP_TOP;
    }
...
static void save_osd_elements_reset_label_text() {
    ...
    lv_obj_set_style_text_font(label_save_osd_elements, &lv_font_montserrat_14, 0);
    save_changes_confirm = CONFIRMATION_UNCONFIRMED;
}

static void cancel_osd_elements_reset_label_text() {
    lv_label_set_text(label_cancel_osd_elements, _lang("Cancel"));
    lv_obj_set_style_text_font(label_cancel_osd_elements, &lv_font_montserrat_14, 0);
...
static int ui_handle_click() {
    ...
    case ROW_RESET_ELEMENTS:
        ...
        if (reset_all_elements_confirm) {
            ...
            update_ui();
            osd_update_element_positions();
            snprintf(buf, sizeof(buf), "#008000 %s.#", _lang("Elements reset"));
...
void ui_osd_element_pos_init(void) {
    ...
    lv_obj_set_style_bg_color(ui_root_container, lv_color_hex(0x101010), 0);
    ...
    // create all elements
    create_btn_group_item_compact(&btn_group_osd_mode, ui_root_container, 2, _lang("Mode"), "4x3", "16x9", "", "", ROW_OSD_MODE, 40, 80, &lv_font_montserrat_14);

    snprintf(buf, sizeof(buf), "%s: ", _lang("Element"));
    create_label_item_compact(ui_root_container, buf, 1, ROW_OSD_ELEMENT, 1, 40, LV_TEXT_ALIGN_LEFT, LV_GRID_ALIGN_START, &lv_font_montserrat_14);
    fill_osd_elements_str();
    dropdown_osd_element = create_dropdown_item(ui_root_container, osd_elements_str, 2, ROW_OSD_ELEMENT, 160, 26, 2, 2, LV_GRID_ALIGN_STRETCH, &lv_font_montserrat_14);

    create_btn_group_item_compact(&btn_group_osd_show_element, ui_root_container, 2, _lang("Show"), _lang("Yes"), _lang("No"), "", "", ROW_OSD_SHOW_ELEMENT, 40, 80, &lv_font_montserrat_14);
    create_slider_item_compact(&slider_group_osd_element_pos_x, ui_root_container, _lang("Pos-X"), OSD_ELEMENT_MAX_X_POS, 0, ROW_OSD_ELEMENT_POS_X, &lv_font_montserrat_14);
    create_slider_item_compact(&slider_group_osd_element_pos_y, ui_root_container, _lang("Pos-Y"), OSD_ELEMENT_MAX_Y_POS, 0, ROW_OSD_ELEMENT_POS_Y, &lv_font_montserrat_14);
    label_save_osd_elements = create_label_item_compact(ui_root_container, _lang("Save changes"), 0, ROW_SAVE, 5, 30, LV_TEXT_ALIGN_CENTER, LV_GRID_ALIGN_CENTER, &lv_font_montserrat_14);
    label_cancel_osd_elements = create_label_item_compact(ui_root_container, _lang("Cancel"), 0, ROW_CANCEL, 5, 30, LV_TEXT_ALIGN_CENTER, LV_GRID_ALIGN_CENTER, &lv_font_montserrat_14);
    label_reset_all_osd_elements = create_label_item_compact(ui_root_container, _lang("Reset all elements (both modes)"), 0, ROW_RESET_ELEMENTS, 5, 30, LV_TEXT_ALIGN_CENTER, LV_GRID_ALIGN_CENTER, &lv_font_montserrat_14);
