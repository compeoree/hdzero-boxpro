...
static lv_coord_t col_dsc[] = {106, 140, 106, 106, 106, 106, LV_GRID_TEMPLATE_LAST};
static lv_coord_t row_dsc[] = {40, 40, 40, 40, 40, 26, 26, 40, 40, 40, LV_GRID_TEMPLATE_LAST};
...
static lv_obj_t *page_elrs_create(lv_obj_t *parent, panel_arr_t *arr) {
    ...
    lv_obj_set_style_pad_top(cancel_label, 8, 0);
...
static void page_elrs_on_click(uint8_t key, int sel) {
    ...
    } else if (sel == POS_WIFI) // start ESP Wifi
    {
        ...
        if (msp_await_resposne(MSP_SET_MODE, 1, (uint8_t *)"P", 1000) != AWAIT_SUCCESS)
            ...
            lv_label_set_text(label_wifi_status, "#008000 Success#");
    } else if (sel == POS_BIND) // start ESP bind
    {
        ...
        if (msp_await_resposne(MSP_SET_MODE, 1, (uint8_t *)"P", 1000) != AWAIT_SUCCESS) {
            ...
            switch (response) {
            case AWAIT_SUCCESS:
                lv_label_set_text(label_bind_status, "#008000 Success#");
                request_uid();
                break;
            case AWAIT_TIMEDOUT:
