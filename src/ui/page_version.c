...
#define ROW_CUR_VERSION        (g_setting.is_pro ? ROW_PRO_CUR_VERSION : ROW_LITE_CUR_VERSION)
#define ROW_RESET_ALL_SETTINGS (g_setting.is_pro ? ROW_PRO_RESET_ALL_SETTINGS : ROW_LITE_RESET_ALL_SETTINGS)
#define ROW_UPDATE_GOGGLE      (g_setting.is_pro ? ROW_PRO_UPDATE_GOGGLE : ROW_LITE_UPDATE_GOGGLE)
#define ROW_UPDATE_ESP32       ROW_PRO_UPDATE_ESP32
#define ROW_BACK               (g_setting.is_pro ? ROW_PRO_BACK : ROW_LITE_BACK)
#define ROW_COUNT              (g_setting.is_pro ? ROW_PRO_COUNT : ROW_LITE_COUNT)
...
static lv_coord_t col_dsc[] = {106, 106, 106, 106, 106, 106, 106, LV_GRID_TEMPLATE_LAST};
...
static fw_select_t *fw_select_current = &fw_select_goggle;
...
static void page_version_fw_scan_for_updates() {
    ...
    page_version_get_latest_fw_files(&fw_select_goggle, "HDZERO_BOXPRO", false);

    if (fw_select_goggle.ready) {
        fw_select_goggle.alt_title = "SD Card";
    }
    if (g_setting.wifi.enable && g_setting.is_pro) {
        if (!fw_select_goggle.ready) {
            has_online_goggle_update =
                0 < page_version_get_latest_fw_path("BoxPro", fw_select_goggle.path, sizeof(fw_select_goggle.path)) &&
                0 < page_version_get_latest_fw_files(&fw_select_goggle, ".bin", true);
        }

        if (has_online_goggle_update) {
            lv_label_set_text(label_note, "To view release notes, select Update BoxPro\n"
                                          "then press the Func button to display or hide the release notes.");
        } else if (fw_select_goggle.alt_title) {
            lv_label_set_text(label_note, "Remove HDZERO_BOXPRO binary files from the root of\n"
...
static void page_version_release_notes_show(fw_select_t *fw_select) {
    ...
    if (notes) {
        ...
        if (msg_box_width > 850) {
            msg_box_width = 850;
        } else if (msg_box_width < 400) {
            msg_box_width = 400;
        }
...
static void page_version_fw_select_populate(fw_select_t *fw_select) {
    ...
    if (msg_box_width > 550) {
        msg_box_width = 550;
    } else if (msg_box_width < 400) {
        msg_box_width = 400;
    }

    lv_obj_set_size(fw_select->container, msg_box_width - 50, 160);
    lv_obj_set_width(fw_select->msgbox, msg_box_width);
    lv_obj_set_width(fw_select->dropdown, msg_box_width - 100);
}
...
static void page_version_fw_select_show(const char *title, fw_select_t *fw_select) {
    ...
    lv_obj_set_style_border_color(fw_select->dropdown, lv_palette_main(LV_PALETTE_RED), 0);
}
...
static void page_version_on_roller_fw_select(uint8_t key) {
    ...
    case 0:
        lv_obj_add_style(fw_select_current->dropdown, &style_dropdown, LV_PART_MAIN);
        lv_obj_set_style_border_color(fw_select_current->dropdown, lv_palette_main(LV_PALETTE_RED), 0);
        break;
    default:
        lv_obj_clear_flag(pp_version.p_arr.panel[pp_version.p_arr.cur], LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_style_border_color(fw_select_current->dropdown, lv_color_hex(0x606060), 0);
        break;
    }
}
...
static void page_version_fw_select_create(const char *device, fw_select_t *fw_select, void (*flash)()) {
    static lv_coord_t msgbox_col_dsc[] = {26, 106, 106, 106, 106, 106, 106, LV_GRID_TEMPLATE_LAST};
    static lv_coord_t mbsbox_row_dsc[] = {40, 40, 40, 40, 40, 40, 40, 40, 40, 40, LV_GRID_TEMPLATE_LAST};
    ...
    fw_select->dropdown = create_dropdown_item(fw_select->container, "", 1, 0, 400, 26, 1, 4, LV_GRID_ALIGN_START, &lv_font_montserrat_18);
...
static lv_obj_t *page_version_create(lv_obj_t *parent, panel_arr_t *arr) {
    ...
    btn_goggle = create_label_item(cont, "Update BoxPro", 1, ROW_UPDATE_GOGGLE, 2);
    ...
    lv_obj_set_size(bar_goggle, 210, 12);
    lv_obj_set_style_bg_color(bar_goggle, lv_color_hex(TEXT_COLOR_DEFAULT), LV_PART_INDICATOR);
    ...
    if (g_setting.is_pro) {
        ...
        lv_obj_set_size(bar_esp, 210, 12);
        lv_obj_set_style_bg_color(bar_esp, lv_color_hex(TEXT_COLOR_DEFAULT), LV_PART_INDICATOR);
    ...
    msgbox_update_complete = create_msgbox_item("Update complete", "BoxPro update completed successfully.\nPlease repower BoxPro now.");
    ...
    lv_label_set_text(label_note, "");
    lv_obj_set_style_text_font(label_note, &lv_font_montserrat_14, 0);
    ...
    lv_obj_set_style_pad_top(label_note, 8, 0);
    ...
    page_version_fw_select_create(" ", &fw_select_goggle, flash_goggle);
...
static void page_version_on_click(uint8_t key, int sel) {
    if (!page_version_release_notes_active()) {
        ...
        if (sel == ROW_CUR_VERSION) {
            ...
            page_version_fw_scan_for_updates();
            page_version_fw_select_show("BoxPro Firmware", &fw_select_goggle);
        } else if (g_setting.is_pro) {
            if (sel == ROW_UPDATE_ESP32) { // flash ESP via SD
                ...
                if (ret == ESP_LOADER_SUCCESS)
                    lv_label_set_text(btn_esp, "#008000 Success#");
                else
    ...
    if (is_short) {
        if (!page_version_release_notes_active()) {
            ...
            case ROW_UPDATE_GOGGLE:
                lv_obj_set_style_border_color(fw_select_goggle.dropdown, lv_color_hex(0x606060), 0);
...
void version_update_title() {
    ...
    if (!reboot_flag)
        lv_label_set_text(btn_goggle, "Update BoxPro");
