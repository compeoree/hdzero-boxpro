...
void Source_AV(uint8_t sel) // 0=rtc6715; 1=AV_in
{
    pthread_mutex_lock(&hardware_mutex);
    LCD_display(0);
    I2C_Write(ADDR_FPGA, 0x8C, 0x00);
    ...
    g_hw_stat.av_pal = g_setting.source.analog_format;
...
int AV_in_detect() // return = 1: vtmg to V536 changed
{
    ...
    if (g_hw_stat.source_mode == SOURCE_MODE_UI) { // detect in UI mode
        ...
        if (det_last != det) {
            ...
            g_hw_stat.av_valid = det;
            g_hw_stat.av_pal = (rdat & 0x01) ? 0 : 1;
        }
        ...
        det = ((rdat & 0xC9) == (g_hw_stat.av_pal ? 0x48 : 0x49)) ? 1 : 0;
        ...
        if (det && det_cnt == AV_DET_SWITCH_CNT) {
            ...
            TP2825_Switch_Mode(g_hw_stat.av_pal);
            ...
            g_setting.source.analog_format = g_hw_stat.av_pal;
            ...
            switch (g_hw_stat.av_valid) {
            case 0: // video loss
                if (h_lock || vh_lock) {
                    g_hw_stat.av_valid = 1;
                }
                break;
