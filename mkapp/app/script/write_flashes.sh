...
if [ -e /mnt/extsd/HDZBOXPRO_VA.bin ]
then
    echo "find VA update file, start update"
        filesize2=`ls -l /mnt/extsd/HDZBOXPRO_VA.bin| awk '{print $5}'`
    gpio_export
    gpio_set_reset
    ...
    insmod /mnt/app/ko/w25q128.ko
        ...
        mtd_debug write /dev/mtd10 0 $filesize2 /mnt/extsd/HDZBOXPRO_VA.bin
		rm /tmp/update.ing -rf

		if [ ! -f /mnt/extsd/DONOTREMOVE.txt ]
		then
			rm /mnt/extsd/HDZBOXPRO_VA.bin -rf
		fi
...
if [ -e /mnt/extsd/HDZBOXPRO_RX.bin ]
then
    echo "find RX update file, start update"
        filesize=`ls -l /mnt/extsd/HDZBOXPRO_RX.bin| awk '{print $5}'`
    gpio_export
    gpio_set_reset
		sleep 1
    insmod /mnt/app/ko/w25q128.ko
    ...
    mtd_debug erase /dev/mtd8 0 65536
        mtd_debug write /dev/mtd8 0 $filesize /mnt/extsd/HDZBOXPRO_RX.bin
    mtd_debug erase /dev/mtd9 0 65536
        mtd_debug write /dev/mtd9 0 $filesize /mnt/extsd/HDZBOXPRO_RX.bin

		if [ ! -f /mnt/extsd/DONOTREMOVE.txt ]
		then
			rm /mnt/extsd/HDZBOXPRO_RX.bin -rf
		fi
