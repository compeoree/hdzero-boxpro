...
cp $MKAPP_DIR/hal/HDZBOXPRO_RX.bin HDZBOXPRO_RX-${HAL_RX_VER}.bin 
cp $MKAPP_DIR/hal/HDZBOXPRO_VA.bin HDZBOXPRO_VA-${HAL_VA_VER}.bin 

echo -e "\npacking ota:"
rm $ROOT_DIR/out/HDZERO_BOXPRO-* || true
tar cvf $ROOT_DIR/out/HDZERO_BOXPRO-${OTA_VERSION}.bin \
    hdzgoggle_app_ota-${APP_VERSION}.tar \
    HDZBOXPRO_RX-${HAL_RX_VER}.bin \
    HDZBOXPRO_VA-${HAL_VA_VER}.bin 

echo -e "\ngenerated out/HDZERO_BOXPRO-${OTA_VERSION}.bin"